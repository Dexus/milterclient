...
func (r SimpleResponse) Continue() bool {
	return byte(r) == continueMilter
}

// Define standard responses with no data
const (
	RespAccept   = SimpleResponse(acceptMilter)
	RespContinue = SimpleResponse(continueMilter)
	RespDiscard  = SimpleResponse(discardMilter)
	RespReject   = SimpleResponse(rejectMilter)
	RespTempFail = SimpleResponse(tempFailMilter)
)
...
func (c *CustomResponse) Continue() bool {
	for _, q := range []byte{acceptMilter, discardMilter, rejectMilter, tempFailMilter} {
		if c.code == q {
			return false
		}
	}
